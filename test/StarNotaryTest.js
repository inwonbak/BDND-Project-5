...
contract('StarNotary', accounts => { 
    ...
    describe('can create a star', () => { 
        it('can create a star and get its name', async function () { 
             ...
             const expected = ['awesome star!', "this star was bought for my wife's birthday", "1", "1", "1"]

             assert.deepEqual(await this.contract.tokenIdToStarInfo(starId), expected);
        })
    })
    ...
    describe('buying and selling stars', () => { 
        ...
        describe('user2 can buy a star that was put up for sale', () => { 
            ...
            it('user2 ether balance changed correctly', async function () { 
                ...
                let balanceBefore = await web3.eth.getBalance(user2)
                await this.contract.buyStar(starId, {from: user2, value: overpaidAmount, gasPrice: 0})
                let balanceAfter = await web3.eth.getBalance(user2)

                assert.equal(balanceBefore - balanceAfter, starPrice)
            })
        })
    })
})
